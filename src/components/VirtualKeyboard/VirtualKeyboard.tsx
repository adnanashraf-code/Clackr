...
// Static regex & symbol map defined outside render loop to prevent garbage collection overhead
...
export default function VirtualKeyboard() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const targetKey = rawKey === " " ? "space" : rawKey;

      setPressedKeys((prev) => {
        if (prev.has(targetKey)) return prev; // Avoid redundant state update on key repeat
  ...
  // Determine expected next character to highlight (memoized)
  const nextChar = useMemo(() => {
    ...
    if (status === "running") {
      ...
      if (typedInput.length < activeWord.length) {
        return activeWord[typedInput.length];
      }
      return " "; // Space expected
    }
    return "";
  }, [status, words, currentWordIndex, typedInput]);
  ...
  const shouldHighlight = useCallback(
    (keyLabel: string): boolean => {
      ...
      const isUppercase = UPPERCASE_REGEX.test(nextChar);
      const isSymbol = SYMBOL_REGEX.test(nextChar);
      ...
      if (SYMBOL_MAP[nextChar] === label) return true;
  ...
  if (!mounted) {
    return <div className="hidden lg:block w-full h-[324px] opacity-0" />;
  }
        ...
        >
          <div className="w-max mx-auto p-2 rounded-2xl bg-[#1E1C1A] border border-[#2A2724]/60 shadow-[0_20px_50px_rgba(0,0,0,0.5)] transition-all duration-300">
            ...
            >
              ...
              {/* Row 3: Tab & QWERTY & Backslash & PgDn */}
              <div className="flex gap-1 w-full">
              ...
              {/* Row 5: Shift & ZXCV & Shift & Up Arrow & End */}
              <div className="flex gap-1 w-full">
